...
const SAMPLE = Number(process.env.NOJS_SAMPLE ?? 8);
...
async function readShape(page: Page, path: string): Promise<Shape> {
  ...
  const h1s = (await page.locator('h1').allTextContents())
    ...
    .filter(Boolean);
  const robots =
    (await page.locator('meta[name="robots"]').first().getAttribute('content').catch(() => null)) ?? '';
...
for (const fam of FAMILIES) {
  test(`${fam.name} — server-rendered pages name their subject (no JS)`, async ({ page, request }) => {
    ...
    test.skip(sample.length === 0, `${fam.name} has no URLs in the sitemap (minUrls is 0, so this is allowed)`);
