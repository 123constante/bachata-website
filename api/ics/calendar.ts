// /api/ics/calendar — iCal subscription feed for external calendar apps.
//
// Calendar apps (Google, Apple, Outlook) poll this URL to stay up to date.
// Returns text/calendar (RFC 5545) with all upcoming events in the next 90 days.
//
// Query params:
...
//
// Self-contained file (no relative imports) — mirrors api/embed/calendar.ts.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    res.status(500).send('Feed not configured');
    return;
  }

  const q = req.query;
  const citySlug = asString(q.city_slug);
  const organiserId = asUuid(q.organiser_id);
  ...
  res.setHeader('Content-Disposition', 'inline; filename="bachata-calendar.ics"');
