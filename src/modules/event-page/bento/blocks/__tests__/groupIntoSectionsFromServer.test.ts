...
import {
  groupIntoSectionsFromServer,
  groupIntoSectionsFromItems,
  isDefaultClassTitle,
  isDefaultPartyTitle,
} from '../ScheduleBlock';
import {
  isRenderableTimelessItem,
  isRenderableTimelessSession,
...
describe('groupIntoSectionsFromItems -- masterclass spine', () => {
  ...
  it('gives a TIMED masterclass its own "Masterclass" section too (every masterclass, not only time-less)', () => {
    // Product decision (2026-07): any all-masterclass slot is promoted to its
    // own MASTERCLASS header whether or not it has a start time.
    const sections = groupIntoSectionsFromItems([
      mkSlot(null, 1200, { type: 'masterclass', sectionKind: 'masterclass' }),
    ]);
    expect(sections.length).toBe(1);
    expect(sections[0].label).toBe('Masterclass');
  });

  it('promotes a masterclass between classes to its own section (Class / Masterclass / Class)', () => {
    const sections = groupIntoSectionsFromItems([
      mkSlot(null, 1140, { type: 'class' }),
      mkSlot(null, 1200, { type: 'masterclass', sectionKind: 'masterclass' }),
      mkSlot(null, 1260, { type: 'class' }),
    ]);
    expect(sections.map((s) => s.label)).toEqual(['Class', 'Masterclass', 'Class']);
  });
});
...
describe('isRenderableTimelessItem -- suppress editor detritus', () => {
  ...
  it('rejects a placeholder-titled row with no other content (Class / Party 1 / Masterclass / Social)', () => {
    // Smart Import / seeds fill these default titles; on their own they render
    // a blank "Time TBC" row (SingleRoomScheduleRow suppresses default titles
    // and a muted rank), so they must not clear the gate.
    expect(isRenderableTimelessItem(mkItem({ title: 'Class' }))).toBe(false);
    expect(isRenderableTimelessItem(mkItem({ title: 'Party 1' }))).toBe(false);
    expect(isRenderableTimelessItem(mkItem({ title: 'Masterclass' }))).toBe(false);
    expect(isRenderableTimelessItem(mkItem({ title: 'Social' }))).toBe(false);
  });

  it('rejects a row whose only levels are unrecognised strings (sanitizeLevels strips them)', () => {
    // hasLevels checks sanitizeLevels(item.levels), not the raw array, so a row
    // whose levels contain only non-canonical values renders no rank/level and
    // must not clear the gate on the strength of levels alone.
    expect(isRenderableTimelessItem(mkItem({ levels: ['all_levels'] }))).toBe(false);
    expect(isRenderableTimelessItem(mkItem({ levels: ['open_level'] }))).toBe(true);
  });

  it('accepts a placeholder-titled row that still has a teacher or a level', () => {
    expect(
      isRenderableTimelessItem(mkItem({ title: 'Masterclass', people: [{ profile_id: 'p1' }] })),
    ).toBe(true);
    expect(isRenderableTimelessItem(mkItem({ title: 'Class', levels: ['open_level'] }))).toBe(true);
  });
});

describe('default-title suppression -- the Untitled placeholder', () => {
  it('treats the RPC NULL-title placeholder "Untitled" as a default (so it is not shown)', () => {
    // The gate ADMITS a time-less masterclass that has a teacher even when its
    // title coalesced to "Untitled"; the render side must then suppress the
    // placeholder word rather than print "UNTITLED" as the headline.
    expect(isDefaultClassTitle('Untitled')).toBe(true);
    expect(isDefaultPartyTitle('Untitled')).toBe(true);
    expect(isDefaultClassTitle('untitled')).toBe(true);
  });

  it('still shows a distinctive title', () => {
    expect(isDefaultClassTitle('Ronald y Alba')).toBe(false);
    expect(isDefaultPartyTitle('Halloween Special')).toBe(false);
  });
});

describe('isRenderableTimelessSession -- gate for the override / festival paths', () => {
  const mkSession = (over: Record<string, unknown>) => ({
    id: 's', title: 'Class', type: 'class', day: null, startMins: null, endMins: null,
    programIndex: 0, levels: [], room: null, people: [],
    sectionId: null, sectionKind: null, sectionLabel: null,
    ...over,
  } as unknown as Parameters<typeof isRenderableTimelessSession>[0]);

  it('rejects a built session with only a default/placeholder title and no people or levels', () => {
    expect(isRenderableTimelessSession(mkSession({ title: 'Class' }))).toBe(false);
    expect(isRenderableTimelessSession(mkSession({ title: 'Party' }))).toBe(false);
    expect(isRenderableTimelessSession(mkSession({ title: 'Untitled' }))).toBe(false);
  });

  it('accepts a built session with a real title, a teacher, or a level', () => {
    expect(isRenderableTimelessSession(mkSession({ title: 'Ronald y Alba' }))).toBe(true);
    expect(isRenderableTimelessSession(mkSession({ people: [{ id: 'p1' }] }))).toBe(true);
    expect(isRenderableTimelessSession(mkSession({ levels: ['open_level'] }))).toBe(true);
  });
});
