-- =============================================================================
-- Migration: 20260504020000_expand_occurrence_override_read_path_v2.sql
-- Date:      2026-05-04
-- Purpose:
--   Extend get_event_page_snapshot_v2 to merge occurrence override_payload
--   for music_styles and photo_urls (gallery), in addition to the fields
--   already override-aware (description, poster_url, ticket_url, level,
--   key_times, website, facebook_url, instagram_url).
--
-- CHANGE SUMMARY (2 lines changed):
--   photo_urls  : was COALESCE(v_meta->'gallery', '[]'::jsonb)
--                 now COALESCE(NULLIF(v_override->'photo_urls', 'null'::jsonb),
--                              v_meta->'gallery', '[]'::jsonb)
--   music_styles: was COALESCE(v_meta->'music_styles', '[]'::jsonb)
--                 now COALESCE(NULLIF(v_override->'music_styles', 'null'::jsonb),
--                              v_meta->'music_styles', '[]'::jsonb)
--
-- BACKWARD COMPATIBILITY:
--   Zero-impact on existing rows. Occurrences without override_payload or
--   without photo_urls/music_styles keys fall through to parent event values
--   exactly as today. NULLIF guards handle the "clear override (JSON null)"
--   case defensively.
--
...
BEGIN
  ...
  v_override := COALESCE(v_occ_eff.override_payload, '{}'::jsonb);
  ...
  v_cover_url := COALESCE(
    NULLIF(trim(v_override->>'poster_url'), ''),
    NULLIF(trim(v_event.poster_url), '')
  );
  ...
  IF to_regclass('public.event_attendance') IS NOT NULL AND v_occ_eff_id IS NOT NULL THEN
    ...
    EXECUTE $att$
      SELECT COUNT(*)
        FROM public.event_attendance ea
       WHERE ea.occurrence_id = $1 AND ea.status = 'interested'
    $att$ INTO v_int_count USING v_occ_eff_id;
  ...
  ELSIF to_regclass('public.event_participants') IS NOT NULL THEN
    ...
    EXECUTE $att$
      SELECT COUNT(*)
        FROM public.event_participants ep
       WHERE ep.event_id = $1 AND ep.status = 'interested'
    $att$ INTO v_int_count USING p_event_id;
  ...
  RETURN jsonb_build_object(
    ...
    'event', jsonb_build_object(
      'name',            v_event.name,
      'description',     COALESCE(NULLIF(trim(v_override->>'description'), ''), v_event.description),
      'status',          v_event.lifecycle_status,
      'is_published',    true,
      'created_by',      v_event.created_by,
      'cover_image_url', v_cover_url,
      'hero_image_url',  v_cover_url,
      -- ── v2 (2026-05-04): override-aware gallery ──────────────────────────
      -- Uses occurrence override_payload.photo_urls when set; falls back to
      -- meta_data.gallery (parent event's gallery). NULLIF guards the
      -- "JSON null = clear override" case defensively.
      'photo_urls',      COALESCE(
                           NULLIF(v_override->'photo_urls',    'null'::jsonb),
                           v_meta->'gallery',
                           '[]'::jsonb
                         ),
      -- ── v2 (2026-05-04): override-aware music styles ─────────────────────
      'music_styles',    COALESCE(
                           NULLIF(v_override->'music_styles',  'null'::jsonb),
                           v_meta->'music_styles',
                           '[]'::jsonb
                         ),
      'key_times',       COALESCE(v_override->'key_times', v_meta->'key_times', v_event.key_times),
      'meta_data_public', v_meta_public,
      'level',           COALESCE(NULLIF(trim(v_override->>'level'), ''), v_event.level),
      'payment_methods', CASE
                            WHEN v_override ? 'payment_methods' THEN v_override->'payment_methods'
                            ELSE to_jsonb(v_event.payment_methods)
                         END,
      'type',            v_event.type,
      'actions', jsonb_build_object(
        'ticket_url',    COALESCE(NULLIF(trim(v_override->>'ticket_url'), ''), v_event.ticket_url),
        'website_url',   COALESCE(NULLIF(trim(v_override->>'website'), ''), v_event.website),
        'facebook_url',  COALESCE(NULLIF(trim(v_override->>'facebook_url'), ''), v_event.facebook_url),
        'instagram_url', COALESCE(NULLIF(trim(v_override->>'instagram_url'), ''), v_event.instagram_url),
        'pricing',       COALESCE(v_override->'pricing', v_event.pricing, '{}'::jsonb)
      )
    ),
...
COMMENT ON FUNCTION public.get_event_page_snapshot_v2(uuid, uuid) IS
    'v2 (2026-05-04): added occurrence override_payload merge for photo_urls '
    '(gallery) and music_styles. key_times override was already present. '
    'All other override fields (description, poster_url, ticket_url, level, '
    'website, facebook_url, instagram_url) unchanged.';
