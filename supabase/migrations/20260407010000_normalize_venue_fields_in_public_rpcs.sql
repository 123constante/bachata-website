-- Migration: Normalize venue fields in public RPCs
-- Date: 2026-04-07
-- Scope:
--   1. get_public_venue_by_venues_id: add google_maps_href (COALESCE of link+url),
--      normalize floor_type (extract first element if stored as JSON array).
--   2. get_event_page_snapshot: normalize floor_type in venue block,
--      add google_maps_href alongside google_maps_link.
--
-- Rationale:
--   - floor_type column can contain either a plain text value ("hardwood")
--     or a JSON array ('["hardwood"]'). The frontend asString() helper
--     returns null for arrays, causing silent data loss.
--   - google_maps_url and google_maps_link are redundant columns;
--     the new google_maps_href field coalesces both for consumers.

-- ============================================================================
-- 1. get_public_venue_by_venues_id — add google_maps_href, normalize floor_type
-- ============================================================================
...
-- ============================================================================
-- 2. get_event_page_snapshot — normalize floor_type in venue SELECT INTO
-- ============================================================================
-- The only change vs the previous version (20260407000001) is the venue
-- SELECT INTO block: floor_type is wrapped in a CASE to extract the first
-- element when stored as a JSON-array text (e.g. '["hardwood"]' → 'hardwood').
...
BEGIN
  ...
  IF v_occurrence.venue_id IS NOT NULL THEN
    SELECT id, name, address, postcode, google_maps_link, photo_url, gallery_urls,
           transport_json, description, capacity,
           CASE WHEN floor_type IS NULL THEN NULL
                WHEN floor_type LIKE '[%' THEN (floor_type::jsonb)->>0
                ELSE floor_type
           END AS floor_type,
           facilities_new
      INTO v_venue
  ...
  RETURN jsonb_build_object(
    ...
    'event', jsonb_build_object(
      ...
      'actions', jsonb_build_object(
        ...
        'whatsapp_link',  v_event.meta_data->>'whatsapp_link',
        'tiktok_url',     v_event.meta_data->>'tiktok_url',
        'livestream_url', v_event.meta_data->>'livestream_url'
      )
    ),
