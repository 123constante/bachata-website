-- =============================================================================
-- Migration: Phase 1 — override_payload-aware public read paths
-- Date: 2026-04-27
--
-- Updates two public RPCs to merge calendar_occurrences.override_payload over
-- events.* for fields that don't already have a per-occurrence path elsewhere.
--
-- Override-aware fields:
--   description, level, payment_methods, pricing, poster_url, ticket_url,
--   website, facebook_url, instagram_url, key_times
--
-- Already per-occurrence (NOT touched here):
--   lineup       -> event_profile_links.occurrence_id
--   times        -> calendar_occurrences.instance_start/end
--   is_cancelled -> calendar_occurrences.lifecycle_status
--   venue/city   -> calendar_occurrences.venue_id/city_id
--
-- This migration is a strict no-op until override_payload is populated:
-- override_payload->>'x' returns NULL for missing keys, so COALESCE falls
-- through to events.* exactly as today.
--
-- Verified end-to-end on production 2026-04-27: applied to live DB,
-- override merge confirmed for all fields, then reverted.
-- =============================================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- 1. get_calendar_events — merges poster_url and key_times overrides
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. get_event_page_snapshot_v2 — merges event-level override fields
-- ---------------------------------------------------------------------------
