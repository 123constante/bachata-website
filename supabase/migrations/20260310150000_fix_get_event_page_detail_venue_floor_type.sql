...
BEGIN
      ...
      SELECT
        ...
        jsonb_build_object(
          ...
          'display_name', COALESCE(
            NULLIF(
              trim(
                concat_ws(' ', d.first_name, d.surname)
              ),
              ''
            ),
            'Dancer'
  ...
  v_venue_json := jsonb_build_object(
    ...
    'floor_type', CASE
      ...
      WHEN jsonb_typeof(v_venue_record.floor_type) = 'array' THEN NULLIF(v_venue_record.floor_type ->> 0, '')
      ELSE NULLIF(v_venue_record.floor_type::text, '')
    END,
    'facilities', CASE
