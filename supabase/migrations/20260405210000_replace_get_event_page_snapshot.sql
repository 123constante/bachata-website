-- Replace get_event_page_snapshot with a clean implementation that:
--   • reads from event_attendance (occurrence-scoped counts + current user status)
--   • reads occurrences from calendar_occurrences
--   • resolves venue from venues, timezone from cities
--   • returns the exact JSON shape expected by parseEventPageSnapshot

CREATE OR REPLACE FUNCTION public.get_event_page_snapshot(
  p_event_id     uuid,
...
BEGIN
  ...
  -- -------------------------------------------------------------------------
  -- 2. Resolve effective occurrence
  -- -------------------------------------------------------------------------
  ...
  IF v_occurrence_id IS NULL THEN
    ...
    ORDER BY instance_start NULLS LAST
    LIMIT 1;
  ...
  -- -------------------------------------------------------------------------
  -- 4b. Lineup for effective occurrence (only when an occurrence is resolved)
  -- -------------------------------------------------------------------------
  IF v_occurrence_id IS NOT NULL THEN
    ...
    WITH base_links AS (
      ...
      SELECT DISTINCT ON (profile_id) profile_id, is_primary
      FROM base_links
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
             ...
             'display_name', COALESCE(
               NULLIF(TRIM(COALESCE(tp.first_name, '') || CASE WHEN tp.surname IS NOT NULL AND length(trim(tp.surname)) > 0 THEN ' ' || trim(tp.surname) ELSE '' END), ''),
               tp.id::text
             ),
    ...
    -- Dancers (dancer_profiles for registered, guest_dancer_profiles for guests)
    WITH base_links AS (
      ...
      SELECT DISTINCT ON (profile_id) profile_id, profile_type, is_primary
      FROM base_links
      ORDER BY profile_id, (occurrence_id IS NOT NULL) DESC, created_at ASC
    )
    ...
    FROM (
      SELECT
        jsonb_build_object(
          'id',           dp.id,
          'display_name', COALESCE(
            NULLIF(TRIM(COALESCE(dp.first_name, '') || CASE WHEN dp.surname IS NOT NULL AND length(trim(dp.surname)) > 0 THEN ' ' || trim(dp.surname) ELSE '' END), ''),
            dp.id::text
          ),
      ...
      WHERE d.profile_type = 'dancer'
      UNION ALL
      SELECT
        jsonb_build_object(
          'id',           gdp.id,
          'display_name', COALESCE(
            NULLIF(TRIM(COALESCE(gdp.first_name, '') || CASE WHEN gdp.surname IS NOT NULL AND length(trim(gdp.surname)) > 0 THEN ' ' || trim(gdp.surname) ELSE '' END), ''),
            gdp.id::text
          ),
      ...
      WHERE d.profile_type = 'guest_dancer'
    ) obj;
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
             'id',           vnd.id,
             'display_name', COALESCE(NULLIF(trim(COALESCE(vnd.business_name, '')), ''), vnd.id::text),
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
             'id',           vid.id,
             'display_name', COALESCE(NULLIF(trim(COALESCE(vid.business_name, '')), ''), vid.id::text),
  ...
  -- -------------------------------------------------------------------------
  -- 5. Build occurrences array
  -- -------------------------------------------------------------------------
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'occurrence_id', co.id,
        'starts_at',     co.instance_start,
        'ends_at',       co.instance_end,
        'local_date',    to_char(
                           co.instance_start AT TIME ZONE v_tz,
                           'YYYY-MM-DD'
                         ),
        'timezone',      v_tz,
        'city_id',       co.city_id,
        'venue_id',      co.venue_id,
        'is_cancelled',  (v_event.lifecycle_status = 'cancelled'),
        'is_live',       (co.instance_start <= v_now AND co.instance_end >= v_now),
        'is_past',       (co.instance_end < v_now),
        'is_upcoming',   (co.instance_start > v_now),
        'lineup',        v_empty_lineup
      )
  ...
  -- -------------------------------------------------------------------------
  -- 6. Build occurrence_effective
  -- -------------------------------------------------------------------------
  IF v_occurrence_id IS NOT NULL THEN
    v_occ_eff := jsonb_build_object(
      'occurrence_id', v_occurrence.id,
      'starts_at',     v_occurrence.instance_start,
      'ends_at',       v_occurrence.instance_end,
      'local_date',    to_char(
                         v_occurrence.instance_start AT TIME ZONE v_tz,
                         'YYYY-MM-DD'
                       ),
      'timezone',      v_tz,
      'city_id',       v_occurrence.city_id,
      'venue_id',      v_occurrence.venue_id,
      'is_cancelled',  (v_event.lifecycle_status = 'cancelled'),
      'is_live',       (v_occurrence.instance_start <= v_now AND v_occurrence.instance_end >= v_now),
      'is_past',       (v_occurrence.instance_end < v_now),
      'is_upcoming',   (v_occurrence.instance_start > v_now),
      'lineup',        v_lineup
    );
  ELSE
    v_occ_eff := NULL;
  ...
  -- -------------------------------------------------------------------------
  -- 7. Attendance counts for the effective occurrence
  -- -------------------------------------------------------------------------
  IF v_occurrence_id IS NOT NULL THEN
    SELECT
  ...
  -- -------------------------------------------------------------------------
  -- 9. Assemble and return full snapshot
  -- -------------------------------------------------------------------------
  RETURN jsonb_build_object(
    ...
    'location_default', jsonb_build_object(
      'city', CASE
        WHEN v_city.id IS NOT NULL THEN
          jsonb_build_object(
            'id',   v_city.id,
            'name', v_city.name,
            'slug', v_city.slug
...
GRANT EXECUTE ON FUNCTION public.get_event_page_snapshot(uuid, uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.get_event_page_snapshot(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_event_page_snapshot(uuid, uuid) TO service_role;
