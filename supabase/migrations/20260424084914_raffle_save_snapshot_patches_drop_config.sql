-- =============================================================================
-- Raffle Phase 1 — Commit 6: admin_save_event_v2_impl + admin_get_event_snapshot_v2 patches + raffle_config drop
-- -----------------------------------------------------------------------------
-- Three changes, one carefully-ordered migration.
--
-- 1. admin_save_event_v2_impl — add has_raffle payload parsing, mirroring the
--    has_guestlist pattern exactly. Four additions:
--      - v_has_raffle_input in DECLARE
--      - has_raffle in INSERT column list
--      - COALESCE(v_has_raffle_input, false) in INSERT VALUES
--      - has_raffle = COALESCE(v_has_raffle_input, e.has_raffle) in UPDATE SET
--
-- 2. admin_get_event_snapshot_v2 — remove the 'raffle_config' key from the
--    return jsonb. One line removed. No other changes.
--
-- 3. ALTER TABLE events DROP COLUMN IF EXISTS raffle_config. Must come AFTER
--    step 2 so the old snapshot body (which references e.raffle_config) is
--    already replaced by the new one before the column disappears. Everything
--    runs inside one transaction, so the new bodies and the column drop commit
--    atomically — post-COMMIT a caller only ever sees the fully-patched state.
--
-- Both function bodies are copied verbatim from pg_get_functiondef as read
-- from live pg_proc on 2026-04-24. Only the five approved diffs differ from
-- the source. No whitespace, quote-style, or delimiter drift. $function$
-- delimiters preserved as pg_proc emits them.
--
-- Not touched in this commit:
--   - submit_raffle_entry, get_event_raffle, sync_raffle_flag, admin draw/list RPCs
...
DECLARE
  ...
  -- CHANGE 1 — level input. NULLIF collapses '' to NULL so the UPDATE branch's
  -- COALESCE preserves the existing value for partial patches.
...
BEGIN
  ...
  -- v_end: ALWAYS taken verbatim from v_event->>'end_time'. Never derived from
  -- MAX(instance_end). This is the key semantic shift documented at the head of
  -- this migration — the client (standard or festival) is now the sole author
  -- of events.end_time.
  --
  -- v_start: preferred from v_event->>'start_time' when supplied. When absent
  -- AND an occurrences array is supplied, fall back to MIN(instance_start) for
  -- back-compat with callers that don't explicitly pass start_time.
  ...
  IF v_event_id IS NULL THEN
    ...
    -- CHANGE 2 — INSERT: new `level` column + value. Nullable (NULL = unspecified).
  ...
  ELSE
    ...
    -- CHANGE 3 — UPDATE: preserve existing level when payload omits the field.
...
ALTER TABLE public.events DROP COLUMN IF EXISTS raffle_config;
