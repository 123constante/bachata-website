-- =============================================================================
-- Migration: get_event_page_snapshot_v2 — Phase 2B
-- Date: 2026-04-21
--
-- Adds FOUR new fields to the public event-page snapshot:
--   1. event.level            (new column, Phase 2A)
--   2. event.payment_methods  (existing column; was dead in UI)
--   3. event.type             (existing column; was dead in UI)
--   4. attendance.interested_count (COUNT over event_attendance WHERE status='interested')
--
-- No other logic has changed. Every other expression is replayed byte-for-byte
-- from the live body (portability capture 20260411000001_capture_get_event_page_snapshot_v2.sql,
-- verified unchanged by subsequent migrations through 2026-04-21).
--
-- NOTE on live-body audit: pg_proc / information_schema.routines are not
-- reachable via the anon/publishable key available locally, so the live body
-- was not re-fetched for this migration. Before pasting into Supabase SQL
-- editor, diff the body below against the output of
--   SELECT pg_get_functiondef('public.get_event_page_snapshot_v2'::regproc);
-- to confirm no drift since 2026-04-11.
-- =============================================================================
...
BEGIN
  ...
  -- -- 7. Pre-resolve all lineup persons --
  ...
  FROM (
    ...
    -- Dancers (dancer_profiles -- profile_type = 'dancer')
    ...
    -- Guest dancers (guest_dancer_profiles -- profile_type = 'guest_dancer')
  ...
  -- -- 8. Build occurrences with per-occurrence lineup --
  FOR v_occ IN
    SELECT *
    FROM public.calendar_occurrences
    WHERE event_id = p_event_id
  ...
  RETURN jsonb_build_object(
    ...
    'event', jsonb_build_object(
      ...
      'level',           v_event.level,
      'payment_methods', v_event.payment_methods,
      'type',            v_event.type,
