-- Migration: Fix actions contract mismatch in get_event_page_snapshot
-- Date: 2026-04-07
-- Scope: Add whatsapp_link, tiktok_url, livestream_url to the actions object.
-- These three values were previously only emitted inside meta_data_public.
-- The frontend reads them from actions.*  (useEventPageQuery.ts lines 207-209),
-- causing the WhatsApp, TikTok, and Livestream buttons to never render.
-- No other sections, no signature change, no grant changes.
