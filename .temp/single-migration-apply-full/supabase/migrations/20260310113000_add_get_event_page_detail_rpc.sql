...
BEGIN
  ...
  v_hero_image_url := COALESCE(
    NULLIF(v_event_record.cover_image_url, ''),
    v_event_record.photo_url[1]
  );
  ...
  SELECT COALESCE(
    jsonb_agg(
      ...
      ORDER BY COALESCE(NULLIF(trim(d.dj_name), ''), NULLIF(trim(d.name), ''), d.id)
    ),
    '[]'::jsonb
  )
  INTO v_djs_json
  ...
  v_event_json := jsonb_build_object(
    ...
    'photo_urls', COALESCE(to_jsonb(v_event_record.photo_url), '[]'::jsonb),
  ...
  v_venue_json := jsonb_build_object(
    ...
    'facilities', CASE
      ...
      ELSE '[]'::jsonb
    END,
    'image_url', v_venue_record.photo_url[1]
  );
